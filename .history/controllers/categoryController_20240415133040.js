...
router.get("/categorylist", function (req, res, next) {
  ...
  db.query(sql, (err, data) => {
    res.render("admin/categoryList", {list: data,layout: "layouts/layout_admin"});
  });
});
...
router.get("/categoryedit/:id", function (req, res, next) {
  ...
  let sql = `SELECT idCategory, name, hidden FROM category WHERE idCategory=${id}`; 
  db.query(sql, (err, data) => {
...
router.post("/update", function (req, res, next) {
  ...
  let tl = req.body.name;
  let ah = req.body.hidden;
  db.query(`UPDATE category SET name = ?, hidden = ? WHERE idCategory = ?`,[tl, ah, id],
    (err, data) => {
