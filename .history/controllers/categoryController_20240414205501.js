...
router.post("/add", function (req, res, next) {
  //nhận dữ liệu từ addnew
  let tl = req.body.name;
  let ah = req.body.hidden;

  let category = { name: tl, hidden: ah };
  db.query("INSERT INTO category SET ?", category, (err, data) => { //chèn vào bảng category dữ liệu name và hidden
    if (err) throw err;
...
router.get("/categoryedit/:id", function (req, res, next) {
  ...
  db.query(sql, (err, data) => {
    res.render("admin/categoryEdit", {
      category: data[0], layout: "layouts/layout_admin", });
  });
});
