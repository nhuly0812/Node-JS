...
var router = express.Router(); // tạo một router mới từ Express.
var db = require("../models/database"); //import module database từ đường dẫn
const session = require("express-session");

//định nghĩa tuyến đường là / rồi render ra trang có đường dẫn client/index
router.get("/", function (req, res) {
  res.render("client/index");
});
//để render trang giới thiệu.
...
//để trả về dữ liệu sản phẩm dưới dạng JSON từ cơ sở dữ liệu
router.get("/product/api", (req, res, next) => {
  let sqlBook = `SELECT id, name, description, imageURL, price, idCategory FROM books;`;
    //sử dụng db.query() để thực thi các truy vấn SQL đó vào cơ sở dữ liệu.
    db.query(sqlBook, (err, books) => {
      if (err) throw err;
      res.json(books); //nếu kh có lỗi kết quả trả về dưới dạng JSON
    });
  });

//để trả về dữ liệu loại sản phẩm dưới dạng JSON từ cơ sở dữ liệu.
...
//để render trang danh sách sản phẩm.
...
//để lấy dữ liệu sp theo danh mục từ API và render một trang sản phẩm dựa trên danh mục đó.
router.get("/product/:id/category", async (req, res, next) => {
  const id = req.params.id; //Lấy tham số id từ URL
  const data = await readApi("http://localhost:5000/product/api");
  const category = await readApi("http://localhost:5000/category/api");
  const product = await filterData(id, data); // lọc dữ liệu dựa trên id và data
  ...
  db.query(sql, async (err, data) => {
    ...
    arr.forEach((item) => {
      products.forEach((item2) => {
        if (item === item2.id) {        // So sánh id sản phẩm trong giỏ hàng với id sản phẩm từ dữ liệu sản phẩm
          arrNew.push(item2);
        }
      });
    });
